...
run this way before being written down, and the output shown is the
real output, not a prediction.

## Trying a one-line expression

You don't need a whole file to try something small. The `-e` option
runs a single piece of code directly from the command line:
...
- Once you've run `bin/setup.8th`, typing `help <word>` or
  `apropos <word>` at the interactive prompt searches that same manual
  for you, without leaving the terminal.
